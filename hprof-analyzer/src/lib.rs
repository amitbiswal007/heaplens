...
struct OverAllocatedCollectionInfo {
    class_name: String,
    size: u32,
    capacity: u32,
}
...
pub struct WasteRawData {
    ...
    array_element_counts: HashMap<u64, u32>,
    over_allocated_collections: Vec<OverAllocatedCollectionInfo>,
    boxed_primitives: Vec<BoxedPrimitiveInfo>,
}
...
fn compute_waste_analysis(
    ...
    // Group by class_name, compute wasted bytes as (capacity - size) * ref_size (assume 8 bytes per ref)
    let ref_size = 8u64; // reference size on 64-bit JVMs
    let mut overalloc_groups: HashMap<String, (u64, u64, f64)> = HashMap::new(); // (count, wasted, sum_fill_ratios)
    ...
    // --- Boxed primitives ---
    // Map class names to their primitive sizes for wasted-bytes calculation
    ...
    let mut boxed_groups: HashMap<String, (u64, u64)> = HashMap::new(); // (count, total_shallow)
